...
\def\dodefinevimtyping[#1][#2]% 
  {\getparameters[\????vimtyping#1][\s!parent=\????vimtyping,#2]%
   ...
   \doifmode\s!mkiv{\setups{vimlinenumbering:mkiv}}%
...
\startsetups[vimsetup]
  ...
  \def\tab##1%
    {\dorecurse{##1}{\space}}%
   \doifmode\s!mkii{\setups{vimlinenumbering:mkii}}%
  \doifinset{\externalfilterparameter\c!option}{\v!packed}
    {\setupwhitespace[\v!none,\v!flexible]}%
  \setcatcodetable\externalfilter__catcodes_write
\stopsetups

\startsetups vimlinenumbering:mkiv
...
\startmode[\s!mkii]
\newcount\vimlinenumber
\stopmode

\startsetups vimlinenumbering:mkii
...
% Line numbering behaves differently in MkII
% FIXME: numbercontinue=yes is not working in MkII
